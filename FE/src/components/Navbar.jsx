...
import { transactionOption, firstFontColorBlack, firstFontColorLight, firstBgColorDark, firstBgColorLight, firstBorderColorDark, firstBorderColorLight, secondaryBgColorDark, secondaryBgColorLight } from "../utils/Const";
...
export default function Navbar({ content, onClick, isMobile, toggleStatus, setToggleStatus, formState, setFormState }) {
  ...
  return (
    <>
      {isMobile ? (
        <>
        <div className={`w-full min-h-[50px] flex items-center px-4 relative border-b ${transactionOption} ${
          ...
          <div className="mx-auto">
            <div
              className={`w-[180px] h-[30px] rounded-sm border
                ...
                ${toggleStatus ? `${secondaryBgColorDark} ${firstBorderColorDark} ${firstFontColorBlack}` : `${secondaryBgColorLight} ${firstBorderColorLight} ${firstFontColorLight}`}
              }`}>{content}
            </div>
          </div>
        ...
        <div className={`w-full min-h-[50px] flex items-center px-4 relative border-b ${transactionOption} ${
          ...
          <div className="flex gap-2 absolute left-4 cursor-pointer">
            ...
            <div id="2" className="rounded-full h-[20px] w-[20px] bg-yellow-400 hover:bg-yellow-700 active:bg-yellow-900 transition-colors duration-400 cursor-pointer" onClick={changeFormState}></div>
            <div id="3" className="rounded-full h-[20px] w-[20px] bg-green-600 hover:bg-green-700 active:bg-green-900 transition-colors duration-400 cursor-pointer" onClick={changeFormState}></div>
          </div>
